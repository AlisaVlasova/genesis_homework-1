...
describe("Profile", () => {
  ...
  it("renders loader while pending", async () => {
    ...
    expect(wrapper.findComponent('[data-test="profile-loader"]').exists()).toBe(
      true
    );
  });
  ...
  it("renders error on request failure", async () => {
    ...
    expect(wrapper.findComponent('[data-test="profile-error"]').exists()).toBe(
      true
    );
  });
  ...
  it("renders content on request success", async () => {
    ...
    expect(
      wrapper.findComponent('[data-test="profile-content"]').exists()
    ).toBe(true);
  });
  it("renders user avatar right", async () => {
    const wrapper = factory({ isLoading: false, isError: false });

    const element = wrapper.findComponent('[data-test="profile-avatar"]');

    expect(element.exists()).toBe(true);
    expect(element.attributes("src")).toBe(
      store.getters.getUserInfo.user.avatarLarger
    );
  });
  it("renders user nickname right", async () => {
    const wrapper = factory({ isLoading: false, isError: false });

    const element = wrapper.findComponent('[data-test="profile-nickname"]');

    expect(element.exists()).toBe(true);
    expect(element.text()).toContain(store.getters.getUserInfo.user.nickname);
  });
  it("renders user followers right", async () => {
    const wrapper = factory({ isLoading: false, isError: false });

    const element = wrapper.findComponent('[data-test="profile-followers"]');

    expect(element.exists()).toBe(true);
    expect(element.text()).toContain(
      store.getters.getUserInfo.stats.followerCount
    );
  });
  it("renders user followings right", async () => {
    const wrapper = factory({ isLoading: false, isError: false });

    const element = wrapper.findComponent('[data-test="profile-followings"]');

    expect(element.exists()).toBe(true);
    expect(element.text()).toContain(
      store.getters.getUserInfo.stats.followingCount
    );
  });
  it("renders user hearts right", async () => {
    const wrapper = factory({ isLoading: false, isError: false });

    const element = wrapper.findComponent('[data-test="profile-hearts"]');

    expect(element.exists()).toBe(true);
    expect(element.text()).toContain(
      store.getters.getUserInfo.stats.heartCount
    );
  });
});
